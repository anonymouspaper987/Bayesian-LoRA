...
class _InducingBase(VariationalMixin):
    ...
    def __init__(self, *args, inducing_rows=None, inducing_cols=None, prior_sd=1., init_lamda=1e-4, learn_lamda=True,
                 ...
                 cache_cholesky=False, ensemble_size=8, **kwargs):
        ...
        self.M_w_cache = None   # cache the mean weight
        ...
        self.z_row = nn.Parameter(torch.randn(inducing_rows, self._d_out)* 0.01* inducing_rows ** -0.5 )
        self.z_col = nn.Parameter(torch.randn(inducing_cols, self._d_in)* 0.01 * inducing_cols ** -0.5)
...
class InducingMixinLinear(_InducingBase):
    ...
    def mean_weight(self):
        if self.M_w_cache is not None:
            return self.M_w_cache
        else:
            weight, bias = self.sample_shaped_parameters()
            self.M_w_cache = weight
            return weight
    ...
    def conditional_mean_and_noise(self, u, row_chol, col_chol):
        ...
        noise_term = torch.randn_like(M_w, device=M_w.device, dtype=M_w.dtype)
...
def register_global_inducing_weights_(module, inducing_rows, inducing_cols, cat_dim=0, **inducing_kwargs):
    ...
    class _GlobalInducingModule(InducingMixinLinear, nn.Linear):
