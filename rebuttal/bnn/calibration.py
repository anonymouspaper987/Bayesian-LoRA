...
def calibration_curve(probabilities: np.ndarray, targets: np.ndarray, num_bins: int,
    ...
    if probabilities.ndim == targets.ndim + 1:
        ...
        if top_class_only:
            ...
            # class was observed
            predictions = np.asarray(probabilities.argmax(-1), dtype=targets.dtype)
            ...
            one_hot_targets = np.asarray(targets[..., np.newaxis] == np.arange(num_classes), dtype=targets.dtype)
