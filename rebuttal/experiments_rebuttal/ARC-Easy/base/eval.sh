#!/bin/bash
# Evaluate base model (no adapter) on ARC-Easy
set -e

LOG_DIR="/home/anonymous/rebuttal/Bayesian_lora_peft/rebuttal/ARC-Easy/base"
...
python rebuttal/eval_classification.py \
    ...
    --seed 42 \
    --log_dir "$LOG_DIR" \
    --output_file /home/anonymous/rebuttal/Bayesian_lora_peft/rebuttal/ARC-Easy/base/eval_results.json \
    2>&1 | tee -a "$EVAL_LOG"

echo "[$(date)] === Evaluation finished ===" | tee -a "$EVAL_LOG"
echo "Results:" | tee -a "$EVAL_LOG"
cat /anvil/scratch/anonymous/rebuttal/Bayesian_lora_peft/outputs/rebuttal/ARC-Easy/base/eval_results.json | tee -a "$EVAL_LOG"
