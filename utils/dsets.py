...
# ===== MMLU (CS & Eng & Law & Health) =====



MMLU_DOMAIN_SUBJECTS = {
    "CS": [
...
class MMLUDataset(ClassificationDataset):
    """
    传入大类名（CS / Eng / Law / Health），从 cais/mmlu 加载该大类下所有科目并合并。
    默认返回 validation split，如果不存在则用 test / dev 兜底。
    """

    def __init__(self, tokenizer, name: str, add_space: bool = True,n_labels=5, few_shot=False, max_len: int = 4096):
        domain = name if isinstance(name, str) else str(name)
        subjects = MMLU_DOMAIN_SUBJECTS.get(domain)
        assert subjects, f"[MMLU] Unknown domain: {domain}"

        split_priority = ("validation", "test", "dev")
        split_buckets = {}
        ...
        for sub in subjects:
            ...
            for sp in split_priority:
                ...
                part = load_dataset("cais/mmlu", sub, split=sp)
                split_buckets.setdefault(sp, []).append(part)
                print(f"[MMLU:{domain}] OK: {sub}:{sp} ({len(part)} items)")
                loaded = True
                break
   
            if not loaded:
                print(f"[MMLU:{domain}] WARNING: no usable split for subject '{sub}'")

        assert any(split_buckets.values()), f"[MMLU:{domain}] No usable subjects!"

        # 合并成一个 DatasetDict
        merged = {sp: concatenate_datasets(parts) for sp, parts in split_buckets.items() if parts}
        if "validation" not in merged:
            for sp in split_priority:
                if sp in merged:
                    merged["validation"] = merged[sp]
                    print(f"[MMLU:{domain}] Using '{sp}' as validation.")
                    break

        self.dset = DatasetDict(merged)
        self.tokenizer = tokenizer
        self.max_len = max_len
        self.domain = domain

        # 构造 label tokens (A/B/C/D)
        spc = " " if add_space else ""
        labels = [f"{spc}{chr(ord('A')+i)}" for i in range(4)]
        self.target_ids = tokenizer(labels, return_tensors="pt", add_special_tokens=False).input_ids[:, -1:]
        self.n_labels = 4
        self.preamble = self.zero_shot_preamble

    zero_shot_preamble = """Return the label of the correct answer.
...
Answer:"""
    ...
    def _format_prompts(self, batch):
        prompts = []
        for e in batch:
            q = (e.get("question") or e.get("prompt") or "")[-self.max_len:]
            choices = e.get("choices") or e.get("options")
            assert choices is not None and len(choices) >= 4, "MMLU expects >=4 choices"
            lines = [f"{chr(ord('A')+i)}) {choices[i]}" for i in range(4)]
            prompts.append(self.preamble.format(question=q, choices="\n".join(lines)))
        return prompts

    def clm_collate_fn(self, batch):
        prompts = self._format_prompts(batch)
        classes = t.tensor([self._ans_to_idx(e["answer"]) for e in batch])
...
class WinograndeDataset(ClassificationDataset):
    def __init__(
        ...
        dset = load_dataset("winogrande", name)
        prompt = self.few_shot_preamble if few_shot else self.zero_shot_preamble
        super().__init__(
            dset,
            tokenizer,
